...
for (const name of fs.readdirSync(base).filter((n) => n === "atlas.json")) {
  const path = new URL(name, base),
    atlas = JSON.parse(fs.readFileSync(path));
  let bytes = 0;
  for (const c of atlas.chunks) {
    const compressed = gzipSync(fs.readFileSync(new URL(c.url.split("/").pop(), base)), {
      level: 9,
    });
    c.gzip = c.url + ".gz";
    c.gzipBytes = compressed.length;
    fs.writeFileSync(new URL(c.gzip.split("/").pop(), base), compressed);
